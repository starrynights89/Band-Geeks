...
public class PurchaseOracle implements PurchaseDAO{
	...
	public double removeBookFromCart(Purchase purchase, Book book) {
		...
		try (Connection conn = cu.getConnection()) {
			CallableStatement cstmt = 
					conn.prepareCall("call remove_book_from_cart(?,?,?)");
			cstmt.setInt(1, purchase.getId());
			cstmt.setInt(2, book.getId());
			cstmt.registerOutParameter(3, OracleTypes.NUMBER);
			cstmt.executeUpdate();
	...
	public Purchase getPurchase(int i) {
		...
		try (Connection conn = cu.getConnection()) {
			String sql = "select p.id as id, p.customer_id as cust,"
					+ " p.total as tot, p.status as status,"
					+ "pb.book_id as book, pb.quantity as quan"
					+ " from purchase p left outer join "
					+ "purchase_book pb on p.id = pb.purchase_id"
					+ " where p.id =?";
	...
	public Set<Purchase> getPurchases() {
		...
		try (Connection conn = cu.getConnection()) {
			String sql = "select p.id as id, p.customer_id as cust,"
					+ " p.total as tot, p.status as status,"
					+ " pb.book_id as book, pb.quantity as quan"
					+ " from purchase p full outer join purchase_book"
					+ " pb on p.id = "
